    ...
    public void onUpdate() {
        ...
        if (this.inGround) {
            ...
            if (raytraceresult.entityHit instanceof EntityLivingBase && !(raytraceresult.entityHit instanceof EntityEnderman) && raytraceresult.entityHit != shootingEntity) {
                EntityLivingBase player = (EntityLivingBase) raytraceresult.entityHit;
                float hearts = player.getHealth();
                if ((hearts >= 1.0F) & (hearts < 20.0F)) {
                    ((EntityLivingBase) this.world.rayTraceBlocks(vec3d1, vec3d, false, true, false).entityHit).setHealth(hearts + 1.0F);
                }
            }
