...
public class BlockCorbaPortalFrame extends BlockMod {
    ...
    public static BlockPattern getOrCreatePortalShape() {
        if (portalShape == null) {
            portalShape = FactoryBlockPattern.start().aisle("?vvv?", ">???<", ">???<", ">???<", "?^^^?").where('?', BlockWorldState.hasState(BlockStateMatcher.ANY)).where('^', BlockWorldState.hasState(BlockStateMatcher.forBlock(JourneyBlocks.corbaPortalFrame).where(EYE, Predicates.equalTo(Boolean.TRUE)).where(FACING, Predicates.equalTo(EnumFacing.SOUTH)))).where('>', BlockWorldState.hasState(BlockStateMatcher.forBlock(JourneyBlocks.corbaPortalFrame).where(EYE, Predicates.equalTo(Boolean.TRUE)).where(FACING, Predicates.equalTo(EnumFacing.WEST)))).where('v', BlockWorldState.hasState(BlockStateMatcher.forBlock(JourneyBlocks.corbaPortalFrame).where(EYE, Predicates.equalTo(Boolean.TRUE)).where(FACING, Predicates.equalTo(EnumFacing.NORTH)))).where('<', BlockWorldState.hasState(BlockStateMatcher.forBlock(JourneyBlocks.corbaPortalFrame).where(EYE, Predicates.equalTo(Boolean.TRUE)).where(FACING, Predicates.equalTo(EnumFacing.EAST)))).build();
        }
        return portalShape;
    }
